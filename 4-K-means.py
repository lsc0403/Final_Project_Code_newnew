...
# 加载数据
file_path = 'C-Transformed_IMDB_Reviews.xlsx'
...
# 选择用于聚类的特征
features = data[['Up Vote Cat Encoded', 'Total Vote Cat Encoded', 'Rating Cat Encoded', 'Predicted Class Cat Encoded']]
...
# 查看聚类结果
print(data[['Up Vote Cat', 'Total Vote Cat', 'Rating Cat', 'Predicted Class Cat', 'Cluster']].head())
# 分析每个聚类中的分布
for cluster in range(kmeans.n_clusters): # 假设我们有4个聚类
    ...
    # 计算并输出原始分类特征的最频繁类别
    for column in ['Up Vote Cat', 'Total Vote Cat', 'Rating Cat', 'Predicted Class Cat']:
