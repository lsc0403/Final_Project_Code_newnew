...
def tokenize_and_numericalize_example(example, tokenizer):
    output = tokenizer(example["text"], truncation=True, padding="max_length", max_length=512, return_tensors="pt")
...
# 设置数据格式为torch，选定需要的列
train_data = train_data.with_format(type="torch", columns=["ids", "attention_mask", "label"])
valid_data = valid_data.with_format(type="torch", columns=["ids", "attention_mask", "label"])
test_data = test_data.with_format(type="torch", columns=["ids", "attention_mask", "label"])
...
def get_collate_fn(pad_index):
    def collate_fn(batch):
        ...
        batch_masks = [i["attention_mask"] for i in batch]  # 新增行
        batch_ids = nn.utils.rnn.pad_sequence(batch_ids, padding_value=pad_index, batch_first=True)
        batch_masks = nn.utils.rnn.pad_sequence(batch_masks, padding_value=0, batch_first=True)  # 新增行
        batch_label = [i["label"] for i in batch]
        batch_label = torch.stack(batch_label)
        batch = {"ids": batch_ids, "attention_mask": batch_masks, "label": batch_label}  # 修改这行，添加attention_mask
        return batch
    return collate_fn
