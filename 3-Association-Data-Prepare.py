...
def transform_reviews_data(file_path):
    ...
    data['Probability Cat'] = ['High' if x >= 0.8 else 'Low' for x in data['Probability']]
    ...
    # 保存到新的Excel文件
    output_file_path = 'C-Transformed_IMDB_Reviews.xlsx'
    transformed_data.to_excel(output_file_path, index=False)

    print(f'Transformed data saved to {output_file_path}')

# 执行转换
file_path = 'B-Modified_IMDB_Reviews.xlsx'
