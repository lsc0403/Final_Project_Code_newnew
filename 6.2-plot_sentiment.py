...
def calculate_positive_sentiment_rate(data):
    ...
    df['PositiveSentiment'] = df['PredictedClass'].apply(lambda x: 1 if x == 1 else 0)
    # 按月计算
...
def generate_plot(monthly_sentiment, yearly_sentiment, total_reviews_by_movie, total_sentiment_by_movie):
    ...
    plot(fig, filename='positive_sentiment_rate_over_time.html', auto_open=False, include_plotlyjs='cdn')
